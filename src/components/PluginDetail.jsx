      ...
      if (kind === 'implied') {
        return (
          <div key={dependency.name} className={kind}>
            <Link href={`/${dependency.name}`}>
              <a>{dependency.title} v.{dependency.version} <span className="req">({kind})</span></a>
            </Link>
            <a href="#" onClick={this.showImplied}><span className="req">(what&apos;s this?)</span></a>
          </div>
        );
      }
      return (
    ...
    return labels.map((id) => {
      ...
      return (
        <div className="label-link" key={id}>
          <Link href={`/?labels=${id}`}><a>{text}</a></Link>
        </div>
      );
    });
  }
